...
function play(url){
    ...
    if(idVideo) launchVideo(idVideo);
    else if(idPlaylist) launchPlaylist(idPlaylist);
    else stopVideo();
}
...
function previous(){
    if(state.playlist) player.previousVideo();
    else{
...
function next(triggered){
    if(state.playlist) nextPlaylist(triggered);
    else nextVideo();
}

function nextPlaylist(triggered){
    if(!isLast && triggered) player.nextVideo();
    else if(isLast) nextVideo();
}
